...
# CSV_PATH = os.path.join(os.path.dirname(__file__),"raw_trade_data.csv") 같은 파일에 있을때는 이렇게 해야함
...
st.write("▶st.table (정적인표)")
st.table(df_exchange)

st.markdown("---")

st.header("3)보너스: 무역 원본 데이터 미리보기")
st.write("공용 데이터파일 raw_trade_data.csv를 읽어온 상위 5행입니다.")
